 ...
 * 
 */
package cn.xvkang.validate.code.impl;
...
import cn.xvkang.validate.code.ValidateCodeType;
...
public abstract class AbstractValidateCodeProcessor<C extends ValidateCode> implements ValidateCodeProcessor {
	...
	private void save(ServletWebRequest request, C validateCode) {
		ValidateCode valiCode = new ValidateCode(validateCode.getCode(), validateCode.getExpireTime());
		sessionStrategy.setAttribute(request, getSessionKey(request), valiCode);
	}
	...
	private String getSessionKey(ServletWebRequest request) {
		...
		if (validateCodeType != null && validateCodeType == ValidateCodeType.IMAGE) {
		...
		if (StringUtils.isBlank(mobile)) {
			try {
				mobile = ServletRequestUtils.getRequiredStringParameter(request.getRequest(), "phone");
			} catch (ServletRequestBindingException e) {
				e.printStackTrace();
			}
		}
	...
	public void validate(ServletWebRequest request) {
		...
		if (processorType == ValidateCodeType.IMAGE) {
			sessionStrategy.removeAttribute(request, sessionKey);
		}
	}
