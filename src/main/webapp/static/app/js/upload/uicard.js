...
FileProgress.prototype.setComplete = function (up, info) {
    ...
    if (!isImg) {
        ...
        $(img).on('load', function () {
            ...
            //删除
            var deleteimg = $('<a class="deleteimg" style="color: #337ab7"/>');
            deleteimg.attr('data-key', res.key).attr('data-hash', res.hash).text('删除');
            infoWrapper.append(deleteimg);
            deleteimg.on('click', function () {
                var key = $(this).data('key');
                var imgname = $(this).data('name');
                var hash = $(this).data('hash');
                var hidden = $(this).parents('.Wrapper').parents('.progressContainer');
                console.log(hidden);
                console.log(hash);
                hidden.html('');
                ...
                console.log(delimg);
                console.log($("#infosrc"));
                var iimg = $("#infosrc").children(delimg);
                //console.log(iimg);
                iimg.remove();
                console.log('hello man,a file is delete');
                //console.log(imgname);
                //console.log(page);
                //$.ajax({
                //    type: "post",
                //    url: page + "api/QiNiu/ImgDel?token=" + token,
                //    dataType: "json",
                //    data: { imgurl: key, imgname: imgname, stuId: stuId, taskId: taskId, userId: userId },
