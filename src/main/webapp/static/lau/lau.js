layui.define(['layer', 'laytpl', 'element'], function(exports) {
    ...
    var Layout = function () {
           ...
           // console.log(LAYID);
            ...
            if (menu[0] && !menu.next('.lau-nav-child')[0]) {
                if (menu.hasClass('lau-nav-header')) {
                    var pmenuParent=$(".layui-side-scroll");
                    pmenuParent.animate({scrollTop: 0},100);
                   // menu.parent().siblings().removeClass('lau-open');
                    ...
                    //var scroll_offset = pmenu.offset();
                    //console.log(scroll_offset);
                    target = pmenu.position().top;
                    var pmenuParent=pmenu.parents('.layui-side-scroll');
                    len_to_scroll = target - pmenuParent.position().top;
                    //if (len_to_scroll > pmenuParent.height()) {
                        pmenuParent.animate({scrollTop: len_to_scroll},100);
                    //}
        ...
        $(document).on('click', '.lau-nav-header', function () {
            var _this = $(this);
            _this.next()[0] ? _this.parent().toggleClass('lau-open')
            //.siblings().removeClass('lau-open') 
