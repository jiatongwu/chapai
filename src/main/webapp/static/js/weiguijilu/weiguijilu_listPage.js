...
layui.config({
    ...
    table.on('tool(table)', function (obj) {
        ...
        if (event == 'view') {
            ...
            layer.open({
                ...
                area: ['700px', '550px'],
                title: '编辑短信模板',
                content: contextPath + '/smsTemplate/editPage.html?id=' + data.id,
                end: function () {
                    // tableReload(table,data);
                    // $('button[lay-filter = "query"]').click();
            ...
            layer.confirm('是否删除此模板？', function (index) {
                layer.load(2);
                var config = {
                    headers: {
                ...
                instance.post(contextPath + '/smsTemplate/delete', params, config)
    ...
    $("#add").on("click", function () {
        ...
        var e = layer.open({
            ...
            area: ['700px', '600px'],
            title: '新增短信模板',
            content: contextPath + '/smsTemplate/addPage.html',
            end: function () {
                // tableReload(table,data);
                // layer.load(2);
                // table.reload('table', {
    ...
    table.render({
        elem: '#table',
        height: 'full-300',
        url: contextPath + '/weiguijilu/selectAll',
        where: {
            createTimeAsc: 0
        },
        page: {
        ...
        cols: [[{
            ...
            field: 'base64image', title: '违规汽车图片', width: 150, templet: '#base64imageTpl', unresize: true
        }, {
            field: 'createtime',
            title: '时间',
            templet: function (d) {
